...
/**
 * Makes an HTTP request using the provided browse instance, with automatic retries on 502 errors.
 * @param {object} browseInstance - The instance of the BROWSE class.
 * @param {string} url - The URL to request.
 * @param {object} options - The options for the browseInstance.request call.
 ...
 * @returns {Promise<object>} The successful response object from browseInstance.request.
 * @throws {Error} Throws an error if the request fails after all retries or encounters a non-retryable error.
 */
async function requestWithRetry(browseInstance, url, options, maxRetries = 5, retryDelay = 1000) {
    let retries = 0;
    while (true) { // Loop indefinitely until success or non-retryable error
        try {
            const response = await browseInstance.request(url, options);
            // If request succeeds, return the response
            return response;
        } catch (error) {
            // Check if it's an HTTPError from got, specifically 502, and if retries are left
            if (error instanceof HTTPError && error.response.statusCode === 502 && retries < maxRetries) {
                retries++;
                console.log(`Received Bad Gateway (502) for ${url}. Retrying in ${retryDelay / 1000}s... (${retries}/${maxRetries})`);
                // Wait for the specified delay before the next attempt
                await new Promise(resolve => setTimeout(resolve, retryDelay));
                // Continue the loop to retry
            } else {
                // If it's not a 502 error, or max retries have been reached, re-throw the error
                console.error(`Request failed for ${url} after ${retries} retries or with non-retryable error:`, error.message);
...
async function class_availability(book_params={},params={}){
    ...
    try {
        const response = await requestWithRetry(params.browse, cust_url, options);
...
class IRCTC{
    ...
    async master_passengers(){
        ...
        try {
            // Use the helper function for the request with retries
            const response = await requestWithRetry(params.browse, cust_url, options);

            // Note: CSRF token update might not be needed here if not returned/used subsequently
            // if (response.headers["csrf-token"]) { params.csrf = response.headers["csrf-token"]; }
            ...
            // Handle errors thrown by requestWithRetry
