...
import{BROWSE} from "./utils/browse.mjs";
...
async function answer_captcha(params={},captcha){
    if(Object.prototype.hasOwnProperty.call(params, "gcloud")){
        ...
        try {
            await execFileAsync(params.viu, [params.captcha_path, "-t"]);
        } catch (error) {
            console.error("Error displaying captcha with viu:", error);
        }
...
async function optimized_psg_input_wait(psg_count){
    // Faster wait times for tatkal bookings
    const psg_input_await = {
        1: 10000,  // 10 seconds instead of 20
        2: 12000,  // 12 seconds instead of 20
        3: 15000,  // 15 seconds instead of 25
        4: 18000,  // 18 seconds instead of 25
        5: 20000,  // 20 seconds instead of 30
        6: 22000,  // 22 seconds instead of 30
    }
    const sleep_time = psg_input_await[psg_count];
    console.log(`Sleeping for ${sleep_time * 0.001} seconds for passenger details input (optimized).`);
    await normal_sleep(sleep_time);
    return "Proceeding to Form Submission";
}
...
async function load(params={}){
    ...
    params.diffTm = Math.round((endTm - startTm) / 2)+e;
    await custom_sleep({
        "slot":params.slot,
        "callfrom":"starter",
...
async function login_with_retry(params={}, maxRetries=3) {
    let retryCount = 0;
    while (retryCount < maxRetries) {
        try {
            ...
            console.log(`Login attempt ${retryCount} failed: ${error.message}, retrying...`);
            await normal_sleep(1000); // Short delay between attempts
        }
    }
}
...
async function class_availability(book_params={},params={}){
    ...
    try {
        const response = await params.browse.request(cust_url, options);
        ...
        console.error("Failed to get class availability:", error.message);
        throw error;
    }
}
...
async function boarding_stations(book_params={},params={}){
    ...
    try {
        ...
        console.error("Failed during boarding stations enquiry:", error.message);
        throw error;
    }
}
...
async function booking_form(book_params={},params={}){
    ...
    try {
        ...
        console.error("Failed during booking form submission:", error.message);
        throw error;
    }
}
...
async function confirm_booking_form_with_retry(book_params={}, params={}, maxRetries=3) {
    let retryCount = 0;
    while (retryCount < maxRetries) {
        try {
            return await confirm_booking_form(book_params, params);
        } catch (error) {
            retryCount++;
            if (retryCount === maxRetries) throw error;
            console.log(`Captcha verification attempt ${retryCount} failed: ${error.message}, retrying...`);
            await normal_sleep(1000);
        }
    }
}
...
async function reliable_payment(book_params, params, maxRetries=2) {
    let retryCount = 0;
    while (retryCount < maxRetries) {
        try {
            if (book_params.payment_type === "2") {
            ...
            retryCount++;
            if (retryCount === maxRetries) throw error;
            console.log(`Payment attempt ${retryCount} failed: ${error.message}, retrying...`);
            await normal_sleep(2000); // Short delay between payment attempts
        }
    }
}
...
async function prepare_tatkal_booking(params={}, irctc_instance) {
    // Validate and prepare booking data before tatkal window opens
    ...
    if (["TQ", "PT"].includes(params.quota) && ["2A","3A","EC","CC","3E"].includes(params.class)) {
        irctc_instance.slot = 1;
    }
    else if (["TQ", "PT"].includes(params.quota) && ["FC","SL","2S"].includes(params.class)) {
        irctc_instance.slot = 2;
    }
    
    params.tid = (new Date).getTime().toString(36);
    params.book_form_data = await book_validator(params, irctc_instance);
    
    // Pre-login to have a session ready
    await irctc_instance.starter(true);
    
    // Cache the booking parameters
    irctc_instance.cached_booking_params = params;
    console.log("Tatkal booking data prepared and cached successfully");
    return "Booking data prepared and cached";
}
...
class IRCTC{
    ...
    async master_passengers(){
        ...
        try {
            ...
            console.error("Failed to get master passenger list:", error.message);
            throw error;
        }
    }
    ...
    async book(params={}){
        ...
        // Use existing cached booking parameters if available (from prepare_tatkal_booking)
        if (this.cached_booking_params && 
            this.cached_booking_params.train === params.train &&
            this.cached_booking_params.from === params.from &&
            this.cached_booking_params.to === params.to &&
            this.cached_booking_params.class === params.class &&
            this.cached_booking_params.quota === params.quota) {
            
            console.log("Using cached booking parameters for faster Tatkal booking");
            params.tid = this.cached_booking_params.tid;
            params.book_form_data = this.cached_booking_params.book_form_data;
        } else {
            params.tid = (new Date).getTime().toString(36);
            params.book_form_data = await book_validator(params, this);
        }
        
        // Ensure we have an active session with fresh login
        await this.starter(true);
        
        // Implement retry loop for critical booking steps
        const maxRetries = 3;
        let retryCount = 0;
        let lastError = null;
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                // Use optimized passenger input wait time for Tatkal bookings
                if (["TQ", "PT"].includes(params.quota)) {
                    const originalPsgCount = params.book_form_data.lapAvlRequestDTO[0].passengerList.length;
                    await optimized_psg_input_wait(originalPsgCount);
                } else {
                    await psg_input_wait(params.book_form_data.lapAvlRequestDTO[0].passengerList.length);
                }
                
                await booking_form(params, this);
                await confirm_booking_form_with_retry(params, this, 3);
                
                // Use the more reliable payment function
                const response = await reliable_payment(params, this, 2);
                console.log("Booking successful!");
                return response;
            } catch (error) {
                retryCount++;
                lastError = error;
                console.error(`Booking attempt ${retryCount} failed: ${error.message}`);
                
                if (retryCount < maxRetries && !error.message.includes("Tickets are unavailable")) {
                    console.log(`Retrying booking (attempt ${retryCount+1}/${maxRetries})...`);
                    await normal_sleep(1000);
                } else {
                    break;
                }
    ...
    async prepare_tatkal_booking(params={}){
        return await prepare_tatkal_booking(params, this);
    }
}
